...
pl.legend(loc=legendLocation, numpoints = 1,title="Specimen [-]")
...
# redefine saveString
saveString = "ethz_-_specimen_vd"
###
### Writing fields to file
###
print "\n\nWrite fluid pressure and fracture closure data to file"

for loadFileCounter in range(len(mechTestFiles)):

	print "\n\tWrite fluidPressure and fractureClosure data to file for specimen: %s" %(loadFileCounter+1)

	saveFileName = str(saveString+str(loadFileCounter+1)+"_fluidPressure_vs_fractureClosure_hm1.txt")
	f = open(saveFileName, 'wt')
	try:
	    writer = csv.writer(f)
	    writer.writerow( ('Number', 'Disp', 'P_fluid') )
	    writer.writerow( ('[-]', '[mm]', '[kPa]') )
	    for i in range( len( expPressure[loadFileCounter] ) ):
	        writer.writerow( (
	        	i+1, 
	        	expSensorDisplacement[loadFileCounter][i] - expSensorDisplacement[loadFileCounter][0],
	            expPressure[loadFileCounter][i]) ) 
	finally:
	    f.close()



print "\n\nWrite axial load and fracture closure data to file"

for loadFileCounter in range(len(mechTestFiles)):

	print "\n\tWrite sigmaZ and fractureClosure data to file for specimen: %d" %(loadFileCounter+1)

	saveFileName = str(saveString+str(loadFileCounter+1)+"_sigmaZ_vs_fractureClosure_hm1.txt")
	f = open(saveFileName, 'wt')
	try:
	    writer = csv.writer(f)
	    writer.writerow( ('Number', 'Disp', 'sigmaZ') )
	    writer.writerow( ('[-]', '[mm]', '[MPa]') )
	    for i in range( len( expPressure[loadFileCounter] ) ):
	        writer.writerow( (
	        	i+1, 
	        	expSensorDisplacement[loadFileCounter][i] - expSensorDisplacement[loadFileCounter][0],
	            expSigmaZ[loadFileCounter][i]) ) 
	finally:
	    f.close()



print "\n\nWrite axial load and fluid pressure data to file"

for loadFileCounter in range(len(mechTestFiles)):

	print "\n\tWrite sigmaZ and fluidPressure data to file for specimen: %s" %(loadFileCounter+1)

	saveFileName = str(saveString+str(loadFileCounter+1)+"_sigmaZ_vs_fluidPressure_hm1.txt")
	f = open(saveFileName, 'wt')
	try:
	    writer = csv.writer(f)
	    writer.writerow( ('Number', 'P_fluid', 'sigmaZ') )
	    writer.writerow( ('[-]', '[kPa]', '[MPa]') )
	    for i in range( len( expPressure[loadFileCounter] ) ):
	        writer.writerow( (
	        	i+1, 
	        	expPressure[loadFileCounter][i] - expSensorDisplacement[loadFileCounter][0],
	            expSigmaZ[loadFileCounter][i]) ) 
	finally:
	    f.close()
